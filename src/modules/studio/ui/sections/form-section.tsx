...
const FormSectionSuspense = ({ videoId }: FormSectionProps) => {
	...
	//TBD: Change if deploying outside of VERCEL
	...
	return (
		<>
			<ThumbnailUploadModal open={thumbnailopen} onOpenChange={setThumbnailOpen} videoId={videoId} />
			<Form {...form}>
										...
										<Select onValueChange={field.onChange} defaultValue={field.value ?? undefined}>
											...
											<SelectContent>
												{categories.map((category) => (
													<SelectItem key={category.id} id={category.id} value={category.id}>
														{category.name}
													</SelectItem>
												))}
								...
								<div className='p-4 flex flex-col gap-y-6'>
									...
									<div className='flex justify-between items-center'>
										<div className='flex flex-col gap-y-1'>
											...
											<p className='text-xs'>{snakeCaseToTitle(video.muxStatus || 'preparing')}</p>
										</div>
									</div>
								</div>
							</div>
